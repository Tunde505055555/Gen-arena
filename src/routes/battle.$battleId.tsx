...
function BattlePage() {
  ...
  async function handleSubmit() {
    ...
    if (answer.trim().length < 10 || reasoning.trim().length < 10) {
      toast.error("Answer and reasoning each need at least a full sentence.");
      return;
    }
    if (!/https?:\/\/\S+/.test(sources)) {
      toast.error("Cite at least one http(s) source URL — the contract fetches and verifies it.");
      return;
    }
    setBusy("submit");
    try {
